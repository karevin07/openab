...
| `book_select` | no | When `true`, Discord shows the complete `books/` inventory in paginated 25-option dropdowns before confirmation. Search matches both the directory slug and the first `# ` heading in `books/{slug}/00-index.md`; the heading is also used as the option label when present. `args` must contain the literal `{{book}}` placeholder exactly once; it may be an argument on its own or embedded in a larger one such as `BOOK={{book}}`, and OpenAB substitutes the selected slug after validating the directory exists. Requires `requires_confirmation = true` and a local runner. Default `false`. Directory names are limited to 48 characters of `[a-z0-9-]`, and the command `id` to 27, so the run button's custom id stays inside Discord's 100-character limit; longer directories are not offered. |
...
| `thread_policy` | string | `"new-each-run"` | `"new-each-run"` or `"sticky"`. Sticky requires `id` and persists the thread in `$HOME/.openab/control.db` for Discord builds. |
...
`$HOME/.openab/control.db` (no `config.toml` rewrite, no restart). Missing
