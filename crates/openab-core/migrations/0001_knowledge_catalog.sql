...
VALUES
    ('github_ai_data_weekly', 'scheduled', '📚 GitHub AI & Data Weekly', 'AI、Data 與 GitHub 每週精選', 'https://www.notion.so/example-ai-data-weekly', NULL, '{"source_shape":"hub","retention":true}', 10),
    ('world_stories', 'scheduled', '🌍 World Stories｜世界人物・王室・文化故事', '世界人物、王室與文化故事', 'https://www.notion.so/example-world-stories', 'collection://example-world-stories', '{"source_shape":"database","retention":true}', 20),
    ('weekly_reading_digest', 'scheduled', '📚 週六精選文章｜Example User Reading List', '每週六精選閱讀清單', 'https://www.notion.so/example-weekly-reading', 'collection://example-weekly-reading', '{"source_shape":"database","retention":true}', 30),
    ('project_notes_alpha', 'side_project', '🧰 Example Project Alpha', 'Example project notes and open questions', 'https://www.notion.so/example-project-notes', 'collection://example-project-notes', '{"notion_project":"Project Alpha"}', 40),
    ('project_notes_beta', 'side_project', '🧰 Example Project Beta', 'Example project notes and experiments', 'https://www.notion.so/example-project-notes', 'collection://example-project-notes', '{"notion_project":"Project Beta"}', 50),
    ('personal_reading_list', 'reading_list', '📕 Reading List', '書籍待讀、閱讀中、已讀、期望與評分管理', 'https://www.notion.so/example-reading-list', 'collection://example-reading-list', '{"home_label":"📕 閱讀清單","retention":false}', 60);
...
VALUES
    ...
    ('project_notes_alpha', 'project', 'Project', 'select', '固定為 config.notion_project', '["Project Alpha"]', 1, 1, 20),
    ...
    ('project_notes_beta', 'project', 'Project', 'select', '固定為 config.notion_project', '["Project Beta"]', 1, 1, 20),
...
VALUES
    ...
    ('weekly_reading_digest', 'latest', '🆕 最新文章', 'primary', 'Example User Reading List｜最新文章', '依推薦週次或發表日期由新到舊唯讀列出最新 5 筆實際文章，以 search card contract 回覆。不要修改 Notion。', 10),
    ('weekly_reading_digest', 'search', '🔎 搜尋內容', 'secondary', 'Example User Reading List｜搜尋內容', '依結構化 adapter 與使用者條件使用參數化 SQL 唯讀搜尋，最低推薦度須轉成明確允許集合，最多 5 筆，以 search card contract 回覆。不要修改 Notion。', 20),
    ('weekly_reading_digest', 'synthesis', '📊 跨期整理', 'secondary', 'Example User Reading List｜跨期整理', '唯讀比較最近 3 期內容，整理重複趨勢、獨有主題與值得追蹤的變化，附 Notion 連結。不要修改 Notion。', 30),
    ('weekly_reading_digest', 'retention', '🗑️ 待刪除', 'secondary', 'Example User Reading List｜待刪除', '唯讀查詢 Knowledge Retention Queue 中本來源 Decision = Pending 的項目，依 Delete After 列出最多 5 筆，以 retention card contract 回覆。不要新增 Queue 項目或修改來源。', 40),
...
VALUES
    ...
    ('project_notes_alpha', 'search', 'query', '標題或內容關鍵字', '例如：Bluetooth、供電、鏡頭', 'short', 0, 300, 10),
    ...
    ('project_notes_beta', 'new', 'title', '標題', '例如：規劃第一批 Medium 文章', 'short', 1, 120, 10),
    ...
    ('project_notes_beta', 'new', 'next_step', '下一步／待確認（選填）', '例如：先驗證 8 週 MVP 題目', 'paragraph', 0, 1000, 40),
    ('project_notes_beta', 'new', 'source_url', '相關連結（選填）', '文章、產品、GitHub 或 Notion URL', 'short', 0, 1000, 50),
    ('project_notes_beta', 'search', 'query', '標題或內容關鍵字', '例如：關鍵字、主題、標籤', 'short', 0, 300, 10),
