...
/// Return the directive payload after `[[` and whether a compatibility prefix
/// was consumed.
fn output_directive_after_open(line: &str) -> Option<(&str, bool)> {
    ...
    let prefix = trimmed[..open_pos].trim();
    let label = prefix
        .strip_suffix('：')
        .or_else(|| prefix.strip_suffix(':'))?
        .trim();
    let supported_label = label.is_empty()
        || matches!(label, "圖片" | "圖像" | "影像" | "附件")
        || matches!(
            label.to_ascii_lowercase().as_str(),
            "image" | "images" | "attachment" | "attachments"
        );
    supported_label.then_some((&trimmed[open_pos + 2..], true))
}
...
pub fn parse_output_directives(content: &str) -> (OutputDirectives, String) {
    ...
    for line in content.lines() {
        // Match [[key:value]] at the start of the line. For attachment directives,
        // tolerate a short image/attachment label or a stray colon generated by an
        // agent before the first directive. Other prose prefixes remain visible text.
        if let Some((after_open, has_compatibility_prefix)) = output_directive_after_open(line) {
            if let Some(close_pos) = after_open.find("]]") {
                let inner = &after_open[..close_pos];
                if let Some((key, value)) = inner.split_once(':') {
                    if has_compatibility_prefix && key.trim() != "attach" {
                        break;
                    }
                    match key.trim() {
                        "reply_to" => {
...
mod directive_tests {
    ...
    #[test]
    fn parse_workspace_attachments_tolerates_leading_fullwidth_colon() {
        let input = "：[[attach:artifacts/one.png]]\n[[attach:artifacts/two.png]]\n[[attach:artifacts/three.png]]\n[[attach:artifacts/four.png]]";
        let (directives, content) = parse_output_directives(input);
    ...
    #[test]
    fn parse_workspace_attachment_tolerates_supported_label() {
        let input = "圖片：[[attach:artifacts/preview.png]]\nPreview attached";
        let (directives, content) = parse_output_directives(input);

        assert_eq!(directives.attachments, vec!["artifacts/preview.png"]);
        assert_eq!(content, "Preview attached");
    }

    #[test]
    fn parse_workspace_attachment_keeps_prose_example() {
        let input = "例如：[[attach:artifacts/preview.png]]\nThis is documentation.";
        let (directives, content) = parse_output_directives(input);
    ...
    #[test]
    fn parse_prefixed_non_attachment_directive_is_rejected() {
        let input = "附件：[[reply_to:123456]]\nContent here";
        let (directives, content) = parse_output_directives(input);
