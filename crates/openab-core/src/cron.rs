...
"#;
    ...
    fn validate_cronjobs_accepts_workspace_alias_without_channel() {
        ...
        job.disable_on_success_match = None;
        job.channel.clear();
        job.workspace_alias = Some("example-library".into());
    ...
    fn sticky_store_round_trips_thread_id() {
        ...
        let mut job = test_cron_job();
        job.id = Some("example-library-daily".into());
        job.thread_policy = CronThreadPolicy::Sticky;
        job.disable_on_success = None;
        let bindings = CronBindings {
        ...
        assert_eq!(persisted["example-library-daily"], "thread-123");
    }
    ...
    fn job_applies_to_project_matches_alias_or_channel() {
        ...
        job.channel.clear();
        job.disable_on_success = None;
        assert!(job_applies_to_project(&job, "example-library", "999"));
        assert!(job_applies_to_project(&job, "@example-library", "999"));
...
"#;
        ...
        assert_eq!(job.id.as_deref(), Some("example-library-daily"));
        assert!(!job.enabled);
        assert_eq!(job.workspace_alias.as_deref(), Some("example-library"));
